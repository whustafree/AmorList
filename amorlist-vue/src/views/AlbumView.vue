<template>
  <div class="fade-in">
    <!-- Gradient header -->
    <div class="relative -mx-3 sm:-mx-4 lg:-mx-8 -mt-4 mb-6 px-3 sm:px-4 lg:px-8 pt-16 pb-8"
      ...
      <div class="flex flex-row sm:flex-row md:flex-row gap-4 sm:gap-6 items-end">
        <div class="w-28 h-28 sm:w-40 sm:h-40 md:w-48 md:h-48 lg:w-56 lg:h-56 overflow-hidden shadow-2xl shrink-0 rounded-md"
        ...
        <div class="flex flex-col gap-1 sm:gap-2 text-left min-w-0 flex-1">
          <span class="text-[10px] sm:text-[11px] font-bold text-[#b3b3b3] tracking-widest uppercase">Álbum</span>
          <h1 class="text-lg sm:text-2xl md:text-3xl lg:text-5xl font-black text-white leading-tight truncate">{{ album.name }}</h1>
          <p class="text-xs sm:text-sm text-[#b3b3b3]">{{ album.songs.length }} canciones</p>
          <div class="flex gap-3 mt-1 sm:mt-3">
            <button @click="playAll"
              class="bg-[#1ed760] hover:bg-[#1fdf64] text-black px-5 sm:px-8 py-2 sm:py-2.5 rounded-full font-bold text-xs sm:text-sm transition-all flex items-center gap-2 shadow-lg shadow-[#1ed760]/30">
